var i=-1, intervalID, counter=6, duration, trem, submitButtonRemoved, title="", extents=[], alpha3_countries=[], alpha3_selected="", mapClickKey, mapPointerMoveKey, ELdeactivated=true, started=false, keyFromServer, score=0, startButtonClicked=false;
...
function d_xyz(string) {
  ...
  var key = CryptoJS.enc.Utf8.parse(keyFromServer);
...
function htmlGenerator(){
  ...
  var right_answer = d_xyz(questionnaire.questions[i].answer);
  ...
  if (type == "MB"){
    ...
    //console.log(answers);
    setTimeout(mapCountryUpdate_add(answers), 1);
  }
  else {
    if (cnt_list.length == 1 && (cnt_list[0] == "Mediterranean" || cnt_list[0] == "Europe" || cnt_list[0] == "World")){
      ...
      setTimeout(mapCountryUpdate_add(cnt_list), 1);
  }
...
function startQuestion(){
  ...
  if(i < counter){
    ...
    //console.log(i);
    //setTimeout(startQuestion, 10500);
    window.clearInterval(intervalID);
    ...
    $.ajax({
      type: "POST",
      url: "/migrate/finish/",  //to be changed in map.js
...
function weighted_score(weight, trem) {
    if (trem>15 && trem<=30)
        q_score = weight*1;
    else if (trem>7 && trem<=15)
        q_score = weight*0.9;
    else
        q_score = weight*0.8;
    return q_score;
...
function evaluateAnswer() {
  ...
  if (type == "TF" || type == "MC") {
    right_answer = capitalizeFirstLetter(d_xyz(questionnaire.questions[i].answer)); //needed only in the case of MC, TF are already in all capital
    ...
    if (given_answer == right_answer) {
      ...
      if (type == "TF") score += weighted_score(0.5, trem);
      else if (type == "MC") score += weighted_score(1, trem);
      //the answer is selected and it is right
    }
    //nothing is selected, just show the right answer
      ...
      $('span').filter(function(){return $(this).text() == right_answer;}).css("color", "rgba(255, 102, 0, 1)");
      $('span').filter(function(){return $(this).text() == right_answer;}).css("font-weight", "bold");
    }
    else {
      //the answer is selected and wrong, so highlight both wrong and right
      //$("span:contains('" + right_answer + "')").css("color", "rgba(0, 205, 0, 1)");
      //$("span:contains('" + right_answer + "')").css("font-weight", "bold");
      $('span').filter(function(){return $(this).text() == right_answer;}).css("color", "rgba(0, 205, 0, 1)");
      $('span').filter(function(){return $(this).text() == right_answer;}).css("font-weight", "bold");
    ...
    right_answer = d_xyz(questionnaire.questions[i].answer_code);
    ...
    right_answer = d_xyz(questionnaire.questions[i].answer);
...
$("#startButton").click(function() {
  if (startButtonClicked == false) {
    ...
    $.getJSON("/migrate/game/restart/", function(data) {  //to be changed in map.js
      $("#noConnection").text("");
      ...
      keyFromServer = data.keyFromServer;
...
/****************temp***************/
/*var name;
$.get( "http://geomobile.como.polimi.it/migrate/profile/", function( data ) {
  ParsedElements = $(data);
  name=ParsedElements.find("h2")[0].innerHTML
});
var x=0;
var tSND={q:[],a:[]};
...
$("body").on("click",function(n){
  ...
  if(btn=="submitButton"){
    ...
    if(x==6){
      ...
      $.ajax({
        type: 'POST',
        url: 'https://migratelogs.herokuapp.com/ranking',
        data: JSON.stringify (tSND),
